#include <egraph_xy/interactXY.h>
...
void ControlPlanner::callPlanner(){
  while(ros::ok()){
    ...
    EGraphStatWriter::writeStatsToFile("egraph_xy_stats.csv", first, res.stat_names, res.stat_values);
    first = false;
  }
}
...
ControlPlanner::ControlPlanner(){
  ...
  planner = ros::NodeHandle().serviceClient<egraph_xy::GetXYPlan>("/sbpl_planning/plan_path", true);
