...
bool CollisionSpaceMgr::isValid(RobotState& robot_pose){
    ContBaseState base = robot_pose.base_state();
    RightContArmState r_arm = robot_pose.right_arm();
    LeftContArmState l_arm = robot_pose.left_arm();
    return isValid(base, r_arm, l_arm);
}
...
bool CollisionSpaceMgr::isValidContState(std::vector<double>& l_arm, 
...
// this is really only used for the egraph 3d heuristic. makes a boolean array
// containing all the obstacles
BooleanOccupGrid CollisionSpaceMgr::getObstacleGrid(){
    BooleanOccupGrid grid;
    int dimX, dimY, dimZ;
