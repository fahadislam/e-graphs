...
RobotState StartGoalGenerator::generateRandomState(Region* region_ptr){
    ...
    while (!foundState){
        ...
        l_arm.setShoulderPan(DefaultLeftAngles[0]);
        l_arm.setShoulderLift(DefaultLeftAngles[1]);
        l_arm.setUpperArmRoll(DefaultLeftAngles[2]);
        l_arm.setElbowFlex(DefaultLeftAngles[3]);
        l_arm.setForearmRoll(DefaultLeftAngles[4]);
        l_arm.setWristFlex(DefaultLeftAngles[5]);
        l_arm.setWristRoll(DefaultLeftAngles[6]);
        ...
        vector<double> init_l_arm = DefaultLeftAngles;
