...
int ContArmState::getDiscFreeAngle(double angle) const {
    double free_angle_res = m_params.arm_free_angle_resolution;
    double free_angle = angle;
    int disc_angle = static_cast<int>((normalize_angle_positive(free_angle + 
                                              free_angle_res*0.5))/free_angle_res);
    //ROS_INFO("free angle: %f, converted: %f", free_angle, convertDiscFreeAngleToCont(disc_angle));
    assert(fabs(angles::shortest_angular_distance(free_angle,convertDiscFreeAngleToCont(disc_angle)))<free_angle_res);
    return disc_angle;
}

int ContArmState::getDiscFreeAngle() const {
    return getDiscFreeAngle(m_angles[Joints::UPPER_ARM_ROLL]);
}
